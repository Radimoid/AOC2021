...
namespace AOC2021 {
    class Day13 {
        HashSet<(int x, int y)> dots = null;
        string[] lines = Common.ReadLines("input13.txt");
        int xMax = 0;
        int yMax = 0;

        public void FoldAlongY(int y) {
            HashSet<(int x, int y)> newDots = new HashSet<(int x, int y)>();
            yMax = y;
            foreach (var dot in dots) {
                if (dot.y < y)
                    newDots.Add(dot);
                else if (dot.y > y) 
                    newDots.Add((dot.x, y - (dot.y - y)));                 
            }
            dots = newDots;
        }
                ...
                if (readingDots) {
                    ...
                    if (match.Success) {
                        if (match.Groups[1].Value == "x")
                            FoldAlongX(int.Parse(match.Groups[2].Value));
                        else
                            FoldAlongY(int.Parse(match.Groups[2].Value));
